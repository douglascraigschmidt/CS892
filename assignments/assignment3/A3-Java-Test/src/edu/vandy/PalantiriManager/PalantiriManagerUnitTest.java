...
public class PalantiriManagerUnitTest {
    ...
    public void testPalantiriManager() {
        PalantiriManager palantiriManager = makePalantiri(2, null);
    ...
    public void testAcquire() throws InterruptedException {
        Thread t =
            new BeingThread(new Runnable() {
                @Override
                public void run() {
                    try {
                        PalantiriManager palantiriManager =
                            makePalantiri(2, null);
                        assertEquals(palantiriManager.availablePalantiri(), 2);
                        palantiriManager.acquire(1000);
                        assertEquals(palantiriManager.availablePalantiri(), 1);
                        palantiriManager.acquire(1000);
                        assertEquals(palantiriManager.availablePalantiri(), 0);
                ...
                public void run() {
                    try {
                        PalantiriManager palantiriManager =
                            makePalantiri(2, null);
                        assertEquals(palantiriManager.availablePalantiri(), 2);
                        Palantir palantir1 = palantiriManager.acquire(1000);
                        assertEquals(palantiriManager.availablePalantiri(), 1);
                        Palantir palantir2 = palantiriManager.acquire(1000);
                        assertEquals(palantiriManager.availablePalantiri(), 0);
    ...
    public void testFairConcurrentAccess() {
    	...
    	final PalantiriManager palantiriManager =
            makePalantiri(PERMIT_COUNT,
                          // Create a callback object that is invoked
                          // to change the fairness indicator if the
                          // PalantiriManager is unfair.
                          new Runnable() {
                              @Override
                                  public void run() {
                                  System.out.println("UNFAIR");
                                  // Report error if the PalantiriManager is unfair.
                                  mFailed = true;
                              }});
                    ...
                    public void run() {
                        ...
                        for (int i = 0;
                            ...
                            try { 
                                ...
                                palantir = palantiriManager.acquire(1000);
                            } catch (Exception e) {
                                mInterrupted = true;
                                return;
                            }
